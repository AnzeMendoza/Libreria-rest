...
public interface EditorialRepository extends BaseRepository<Editorial, Integer> {
    ...
    @Query(value = "select * from fn_cambiar_estado_editorial(:id,:status);", nativeQuery = true)
    String changeStatus(@Param("id") int id, @Param("status") Boolean estado);

    @Query(value = "select * from fn_crear_editorial(:name)", nativeQuery = true)
    String saveEditorial(@Param("name") String name);

    @Query(value = "select * from fn_modificar_editorial(:id,:name);", nativeQuery = true)
    String updateEditorial(@Param("id") Integer id, @Param("name") String name);

    @Query(value = "select * from fn_buscar_por_patron_nombre_editorial(:string )", nativeQuery = true)
